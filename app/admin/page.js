...
// ==========================================
// 1. HUB DASHBOARD (ÖZEL İZİN SENKRONİZASYONLU)
// ==========================================
function HubDashboard({ currentUser, userData, onLogout }) {
  ...
  // İzin: canViewAllProjects açıksa tüm stüdyoyu görebilir
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="text-right hidden sm:block">
              <p className="text-xs font-semibold text-slate-800 flex items-center justify-end gap-1">
                {isMasterCEO && <Crown className="w-3.5 h-3.5 text-amber-600" />}
                {userData?.displayName || currentUser?.email}
              </p>
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                Unvan: <span className="text-indigo-600">{displayRankTitle}</span>
              </p>
            </div>

            {/* Yönetim Masası İzni Varsa Buton Görünür */}
            {permissions.canAccessAdmin && (
              ...
              >
                <ShieldAlert className="w-3.5 h-3.5 text-slate-600" /> Yönetim Masası
              </Link>
            )}
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 flex-1 w-full space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
          <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm">
            ...
            <div className="text-2xl font-bold text-slate-900 mt-1">{displayedProjects.length}</div>
          </div>
          <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 pb-3 gap-3">
          ...
          <div className="flex items-center gap-2">
            {/* TÜM PROJELERİ GÖRME İZNİ OLANLARA ÖZEL KAPSAM BUTONU */}
            {permissions.canViewAllProjects && activeTab === "projects" && (
              <div className="flex items-center bg-slate-100 p-0.5 rounded-lg border border-slate-200">
                <button
                  onClick={() => setViewScope("my")}
                  className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-all ${
                    viewScope === "my" ? "bg-white text-slate-900 shadow-xs" : "text-slate-500"
                  }`}
                >
                  Benim İşlerim
                </button>
                <button
                  ...
                  className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-all ${
                    viewScope === "all" ? "bg-white text-slate-900 shadow-xs" : "text-slate-500"
                  }`}
                >
                  Tüm Stüdyo (Genişletilmiş)
                </button>
              </div>
            )}
          ...
          <div>
            ...
            {filteredProjects.length === 0 ? (
              <div className="p-12 rounded-2xl bg-white border border-slate-200 text-center shadow-sm">
                ...
                <h3 className="text-sm font-semibold text-slate-800">Gösterilecek iş bulunamadı</h3>
                <p className="text-xs text-slate-500 mt-1 mb-4">Yeni bir Script, 3D Model, Müzik veya Build çalışması ekleyin.</p>
                <button
                  onClick={() => setShowProjectModal(true)}
    ...
    <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center p-4 sm:p-6 font-sans">
      <div className="max-w-xl w-full">
        <div className="text-center mb-6">
          ...
          <h1 className="text-xl font-bold text-slate-900 tracking-tight">Stüdyo Ekip Başvuru Formu</h1>
          <p className="text-xs text-slate-500 mt-0.5">True Kinetic Studios Katılım Başvurusu</p>
        </div>
            ...
            <div>
              ...
              {appForm.roleApplied === "Diğer" && (
                <div className="mt-2.5 p-3 rounded-xl bg-slate-50 border border-slate-200">
                  <label className="block text-slate-700 font-semibold mb-1">Hangi rolde başvuruyorsunuz? *</label>
                  <input
                    type="text"
                    required
